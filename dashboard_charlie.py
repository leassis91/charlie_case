...
def data_overview(df):
    ...
    with c1:
        aux4 = df.groupby('day_of_week')['id'].count().sort_values(ascending=False).reset_index()
        fig = px.bar(aux4, 
                x='day_of_week', 
                y='id',
                color='day_of_week',
    ...
    with c2:
        aux5 = df.groupby('day_of_week')['receita'].sum().sort_values(ascending=False).reset_index()
        fig = px.bar(aux5, 
                x='day_of_week',
                y='receita',
